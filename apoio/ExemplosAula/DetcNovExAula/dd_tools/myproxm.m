...
if ischar(type)
	...
	all = char('polynomial','p','exponential','e','radial_basis','r', 'radial_basis_adapt','ra', ...
...
% Execution, input data A and W.A, output in D (-->W)
	...
	case {'radial_basis_adapt','ra'}
      
%         distmat = distm(+A,+W.A);
%         sD = sort(distmat);
%         for cont1=1:length(+A)
%             for cont2=1:length(+W.A)
%                 sigmaij(cont1,cont2) = ((sqrt(sD(W.s+1,cont1)) + sqrt(sD(W.s+1,cont2)))/2);
%                 D(cont1,cont2) = exp(-distm(+A(cont1,:),+W.A(cont2,:))/(sigmaij(cont1,cont2)*sigmaij(cont1,cont2)));
%             end
%         end   
        
        distmat = distm(+A,+W.A);
        sD1 = sort(distmat'); 
        distmat = distm(+W.A,+W.A);
        sD2 = sort(distmat'); 
        
        for cont1=1:length(+A)
            for cont2=1:length(+W.A)
                sigmaij(cont1,cont2) = ((sqrt(sD1(W.s+1,cont1)) + sqrt(sD2(W.s+1,cont2)))/2);
                D(cont1,cont2) = exp(-distm(+A(cont1,:),+W.A(cont2,:))/(sigmaij(cont1,cont2)*sigmaij(cont1,cont2)));
            end
        end        
