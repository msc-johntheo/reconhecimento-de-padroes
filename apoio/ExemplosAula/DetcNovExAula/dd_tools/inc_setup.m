...
switch itype
    case 'svdd'
        ...
        x(It(1:N),:) = [];
        y(It(1:N),:) = [];
        if (W.C>=1)  % the 'hard' svdd, one obj. becomes SV:
            W.setS = 1;
            ...
            if abs(1-N*W.C)<eps, % very rare situation, troublesome:
                W.C = W.C+10*eps;
            end
            set0 = (1:N)';
            W.alf(set0,1) = [repmat(W.C,N-1,1); 1-(N-1)*W.C];
